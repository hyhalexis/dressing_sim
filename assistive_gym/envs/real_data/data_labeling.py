...
def get_label(vlm_dict_list_1, vlm_dict_list_2, batch_number):
    ...
    save_path = '/scratch/alexis/data/data_collection_trajs/labeled'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
...
def get_vlm_label(vlm_dict_list_1, vlm_dict_list_2):
    ...
    for epoch in range(num_epochs):
        ...
        acc1 = get_label(dict_list_1, dict_list_2, epoch)
...
def sample_v_dict(trajs, batch_size=4000):
    ...
    for i in sample_batch:
        ...
        while True:
            ...
            if abs(sample_index[0] - sample_index[1]) > 5:
                break
        ...
        obs1_pos = traj[sample_index[0]]["obs_pos"]
        obs1_x = traj[sample_index[0]]["obs_x"]
        
        obs2_pos = traj[sample_index[1]]["obs_pos"]
        obs2_x = traj[sample_index[1]]["obs_x"]

        dict_1["obs"] = Data(pos=torch.from_numpy(obs1_pos).float(), x=torch.from_numpy(obs1_x).float())
        dict_2["obs"] = Data(pos=torch.from_numpy(obs2_pos).float(), x=torch.from_numpy(obs2_x).float())
...
def train_reward_model(data_dir="/scratch/alexis/data/data_collection_trajs"): 
