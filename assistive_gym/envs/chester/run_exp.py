...
class VariantGenerator(object):
    ...
    def add(self, key, vals, **kwargs):
        for i, (k, v, kw) in enumerate(self._variants):
            if k == key:
                self._variants[i] = (key, vals, kwargs)
                return
...
def rsync_code(remote_host, remote_dir):
    os.system(
        'rsync -avzh --include-from=\'./chester/rsync_include\' --exclude-from=\'./chester/rsync_exclude\'   ./ ' + remote_host + ':' + remote_dir)
...
def run_experiment_lite(
  ...
  **kwargs):
    ...
    :param
    """
    last_variant = variants[0].pop('chester_last_variant', False)
    if mode == 'singularity':
    ...
    if batch_tasks is None:
        ...
        # print(batch_tasks[0])
        # print(batch_tasks[0]['variant'].get("pre_commands"))
        # # print(batch_tasks[1])
        # print(batch_tasks[1]['variant'].get("pre_commands"))
        # exit()
    ...
    for task_idx, task in enumerate(batch_tasks):
        variant = variants[task_idx]
        call = task.pop("stub_method_call")
        if use_cloudpickle:
            import cloudpickle
            data = base64.b64encode(cloudpickle.dumps(call, protocol=4)).decode("utf-8")
        else:
        ...
        if task.get("exp_name", None) is None:
            ...
            #         exp_name += '_{}={}'.format(v, variant[v])
            #     elif isinstance(variant[v], bool):
            #         if variant[v]:
            #             exp_name += '_{}'.format(v)
            #     else:
            #         exp_name += '_{}={:g}'.format(v, variant[v])
            ...
            # else:
            #     ind = 10
            print('exp full name ', exp_name)
            # if exp_count == -1:
            #     exp_count = ind + 1
            task["exp_name"] = "{}-{}-{:03}".format(exp_name, timestamp, exp_count)
    ...
    if mode == "local":
        ...
        for task in batch_tasks:
            ...
            try:
                ...
                if isinstance(e, KeyboardInterrupt):
                    raise
            return popen_obj
    elif mode in ['seuss', 'psc']:
        num_required_slurm_gpus = len(batch_tasks) // task_per_gpu
        if len(batch_tasks) % task_per_gpu != 0:
            num_required_slurm_gpus += 1

        for slurm_task_idx in range(num_required_slurm_gpus):
        # for task in batch_tasks:
            beg_idx = slurm_task_idx*task_per_gpu
            end_idx = min(len(batch_tasks), (slurm_task_idx+1)*task_per_gpu)
            tasks = batch_tasks[beg_idx: end_idx]
            task = tasks[0]
            ...
            data_dir = config.REMOTE_LOG_DIR[mode] + "/local/" + exp_prefix + "/" + task["exp_name"]
        ...
        for task in batch_tasks:
            ...
            header = header + "\n#CHESTEROUT " + os.path.join(data_dir, 'chester.out')
            header = header + "\n#CHESTERERR " + os.path.join(data_dir, 'chester.err')
            ...
            if last_variant:
                ...
                log_file = os.path.join(config.CHESTER_CHEDULER_LOG_DIR, '{}.txt'.format(task['exp_name']))
